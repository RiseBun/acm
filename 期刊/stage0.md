...
- **Persist tokenisation to disk** (pickle or `.pt`). First seed still pays the ~10 min cost, later seeds become instantaneous. ~20 min / 3-seed run saved.
- `num_workers=4` + `persistent_workers=True` in the `DataLoader`s (`run_stage0_table3.py:163-177`). Expected GPU utilisation 5 % → 30-50 %.
- Push `batch_size` 32 → 256. H20 can fit > 1 k for this model; linear-scale the LR.
