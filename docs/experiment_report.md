...
| variants | `object_only`, `object_relation`, `object_relation_decoupled_visibility`, `object_relation_decoupled` |
...
- `experiments/nuplan_stage0_20k_seed7/object_relation/table3_results.json`
...
| object_relation | 239.025 | 4.313 | 0.508 | 24.376 | 0.132 |
...
结论：nuPlan 20k Stage 0 仍然极强地支持 decoupled 表示。新增的
`object_relation` shared top-k baseline 反而比 object-only 更差
(DynRoll 239.0, RareADE 24.4, IntRec@1m 0.132)，说明 naive shared
relation/object mixing 在 nuPlan 上也不能作为充分表示。
...
| split | 16,000 train / 4,000 val |
| seeds | 7, 42, 123 |
| conditions | `wm_object`, `wm_decoupled`, `wm_decoupled_no_vis`; follow-up adds `wm_naive` |
| epochs | 10 |
| batch size | 128 |
| horizon | 5 |
...
| wm_naive (shared relation) | -2.66 ± 14.63 | 0.252 ± 0.050 | 0.297 ± 0.069 | 0.186 ± 0.074 | 0.017 ± 0.012 |
...
| Seed | Return | CollRate | CollMean | Stab(ego-cos) | Stab(L2) |
|---:|---:|---:|---:|---:|---:|
| 7 | -15.597 | 0.263 | 0.320 | 0.197 | 0.004 |
| 42 | 13.222 | 0.198 | 0.219 | 0.254 | 0.028 |
| 123 | -5.591 | 0.297 | 0.352 | 0.106 | 0.019 |
...
3. 新增 `wm_naive` shared-relation baseline 后，机制链条更完整：
   naive shared top-k 在 Stage 0 表示上失败，在 Stage 1 下游上也高方差
   且 return 远低于 typed decoupled no-vis。
4. `wm_decoupled(+vis)` Return 高于 object-only，但 collision 更差，并且 seed 123 不稳定。
5. visibility 的角色呈现数据集依赖：nuScenes Stage 1 中 visibility 是 stabilizer；nuPlan 20k 中 no-vis 反而最强。
...
外部非同族 PlanTF baseline 也已完成最小 nuPlan-devkit 接入 sanity：使用下载好的 `planTF.ckpt`、固定 13 个 `one_of_each_scenario_type` tokens、20-step closed-loop nonreactive simulation，禁用重型 official metrics 和 simulation-log 压缩后，`runner_report.parquet` 记录 13/13 场景成功完成。另有 1-scenario official-metric smoke：`single_right_turn_h20_20step_metric_nosimlog` 成功产出 aggregator parquet，但 final score 为 0.0，drivable/progress/comfort/collision/TTC 等聚合项均为 0。结果整理在 `experiments/plantf_external_baseline_nuplan/summary.{md,json}`。这个结果证明外部 checkpoint / planner wrapper 能跑通，也证明 official metric 路径可产出文件；但运行时间和得分主要被 nuPlan DB、map feature、metric plumbing / wrapper 行为主导，不应作为主文 official closed-loop benchmark；A1 主线仍建议采用文档中推荐的低成本 BC / planner-target imitation baseline。

A1 的低成本 BC / planner-target imitation baseline 已按 nuPlan 50k Stage1 协议完成 3 seeds：复用 `run_stage1_table4.py --condition bc`、`object_only` 表示、Stage0 `object_only` warm-start、10 epochs、batch 128、horizon 5。聚合结果在 `experiments/nuplan_bc_baseline_50k/summary.{md,json}`：Return 0.43 ± 4.23，ImagColl 0.327 ± 0.566，CollMean 0.479 ± 0.368。offline planner sanity 也已补齐：teacher action MSE 8.824 ± 0.113，teacher action delta L2 3.534 ± 0.017。它能作为 reviewer-facing 的外部风格锚点，但明显弱于 `wm_decoupled_no_vis` 50k 主条件。

B1 typed-budget sensitivity 已补 `10/6`：`experiments/stage0_budget_10_6_nuscenes/summary.{md,json}`。在 nuScenes Stage0 三 seed 上，10/6 的 DynRoll 8.761 ± 1.067、RareADE 1.574 ± 0.485，明显弱于 12/4 主条件的 DynRoll 1.876 ± 0.227、RareADE 0.520 ± 0.049；IntRec@1m 也从 0.979 降到 0.884。这支持 12/4 是更干净的默认 typed budget，而不是孤立拍出来的点。

B2 relation feature-group ablation 已完成轻量 evaluation-time probe：`experiments/nuplan_relation_feature_ablation_50k/summary.{md,json}`。对 `wm_decoupled_no_vis` 的 50k checkpoint，在验证时 mask relation-token `TTC/risk` 会让 CollRate 从 0.260 升到 0.895、CollMean 从 0.277 升到 0.891、Return 从 14.51 降到 8.50；mask `lane_conflict/priority` 基本中性。这个结果说明当前 no-vis 优势主要来自 TTC/risk 风险关系，而不是 lane/priority 字段。
...
| nuPlan 50k BC / planner-target imitation | Decoupled-no-vis remains stronger | BC Return 0.43 ± 4.23, CollRate 0.327 ± 0.566, CollMean 0.479 ± 0.368, teacher MSE 8.824 ± 0.113 | low-cost external-style A1 anchor is runnable but not competitive |
...
| nuScenes Stage0 typed-budget sensitivity | 12/4 remains cleaner than 10/6 | 10/6 DynRoll 8.761 ± 1.067, RareADE 1.574 ± 0.485 vs 12/4 DynRoll 1.876 ± 0.227, RareADE 0.520 ± 0.049 | appendix sensitivity; relation-heavy budget is not better |
| nuPlan relation feature-group ablation | TTC/risk is the critical relation group | Masking TTC/risk raises CollRate 0.260 -> 0.895; masking lane/priority is near-neutral | mechanism support for risk-relation interpretation |
...
| External PlanTF nuPlan-devkit wrapper | PlanTF checkpoint runs end-to-end on 13 fixed nuPlan `one_of_each` scenarios | 13/13 successful 20-step closed-loop runs with metrics disabled; summary under `experiments/plantf_external_baseline_nuplan/` | useful external non-family integration anchor; official metrics are too dominated by nuPlan DB/metric overhead for a main claim |
