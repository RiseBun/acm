...
- A1 external-style BC / planner-target imitation baseline：完成 nuPlan 50k 三 seed，并补齐 offline teacher-action 指标。结果在 `experiments/nuplan_bc_baseline_50k/summary.{md,json}`。
- A2 shared-relation Stage1 baseline：完成 nuPlan 20k 三 seed。结果在 `experiments/nuplan_stage1_shared_relation_20k/summary.{md,json}`。
- B1 `10/6` typed-budget sensitivity：完成 nuScenes Stage0 三 seed。结果在 `experiments/stage0_budget_10_6_nuscenes/summary.{md,json}`。
- B2 relation feature-group ablation：完成 nuPlan 50k evaluation-time probe。结果在 `experiments/nuplan_relation_feature_ablation_50k/summary.{md,json}`。
- PlanTF external checkpoint sanity：已跑通 nuPlan-devkit wrapper，作为 appendix / integration proof。结果在 `experiments/plantf_external_baseline_nuplan/summary.{md,json}`。

最关键新增结论：

- BC baseline 可作为外部风格锚点，但不具竞争力：Return 0.43 ± 4.23，teacher action MSE 8.824 ± 0.113。
- 10/6 relation-heavy budget 弱于 12/4：DynRoll 8.761 ± 1.067 vs 1.876 ± 0.227，说明 12/4 不是孤立拍出来的点。
- `wm_decoupled_no_vis` 的 relation 收益主要来自 TTC/risk：mask TTC/risk 后 CollRate 0.260 -> 0.895；mask lane/priority 近似中性。
